...
class pdf2txt():
    ...
    def convert_pdfs(self, to_json=False):
        for search in self.search_list:
            ...
            self.convertMultiple(os.path.join(self.download_dir, search, ""), os.path.join(self.txt_dir, search, ""), to_json=to_json)
            print("")
        ...
        for i, pdf in enumerate(os.listdir(pdfDir)):  # iterate through pdfs in pdf directory
            ...
            if fileExtension == "pdf":
                try:
                    ...
                    out = out.append([pd.DataFrame([[pdfFilename, self.titlelist[int(pdf[:-4])-1], text]], columns=['Filename', 'Title', 'Content'])], ignore_index=True)
